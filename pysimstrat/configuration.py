...
class Parfile(ParfileType):
	...
	def read(self):
		...
		with open(fpath, 'r', encoding='utf-8') as parfile:
...
def simstrat_outputfoldertype():
	class OutputFolder():
		def __init__(self, filename, content=None, root=None):
			pass
...
def simstrat_listtype(columns, float_formats, parname):
	...
	class InputList(SimstratListType):
		def __init__(self, filename, content=None, root='./'):
		...
		def read(self):
			df = pd.read_csv(os.path.join(self.root, self.filename), sep='\t', header=0)
		...
		def write(self):
			...
			if data is None:
				with open(os.path.join(self.root, self.filename), 'w', encoding='utf-8') as inputfile:
				...
				data.to_csv(os.path.join(self.root, self.filename), sep='\t', index=False, encoding='utf-8')
...
def simstrat_tabulartype(index_name, header_name, parname):
	...
	class InputTabular(SimstratTabularType):
		...
		def read(self):
			self.df = pd.read_csv(os.path.join(self.root, self.filename), sep='\t', skiprows=2, header=0, index_col=0)
		...
		def write(self):
			if self.df is None:
				...
				self.df.to_csv(os.path.join(self.root, self.filename), sep='\t', encoding='utf-8')
				with open(os.path.join(self.root, self.filename), 'r', encoding='utf-8') as inputfile:
...
class SimstratConfiguration(object):
	def __init__(self, parfile_name, root='./'):
		assert os.path.isfile(os.path.join(root, parfile_name))
	...
	def readConfiguration(self):
		self.parfile = Parfile(self.parfile_name, root=self.root)
		self.parfile.read()
		...
		for inputfile in inputfile_paths:
			setattr(self, inputfile, inputfile_classes[inputfile].value(inputfile_paths[inputfile], root=self.root))
...
class DefaultSimstratConfiguration(SimstratConfiguration):
	def __init__(self, lakename, depth, startdate, enddate, absorption=0.2, root='./'):
		# generate filenames
		...
		infolderpath = os.path.join(self.root, inputfolder)
		outfolderpath = os.path.join(self.root, outputfolder)
		if not os.path.isdir(infolderpath):
			os.mkdir(infolderpath)
		...
		inputfile_classes = SimstratInputFileDefinitions.__members__
		for inputfile in inputfile_paths:
			setattr(self, inputfile, inputfile_classes[inputfile].value(inputfile_paths[inputfile], content=DEFAULTS[inputfile], root=self.root))
