...
def generateTemplate(templatedefinition, marker, root='./'):
	...
	class InputfileTemplate(inputfile_class):
		...
		def write(self):
			...
			with open(os.path.join(self.root, self.filename), 'r', encoding='utf-8') as templatefile:
				content = templatefile.read()

			with open(os.path.join(self.root, self.filename), 'w', encoding='utf-8') as templatefile:
...
def generateOutputInstructions(outputinstructiondefinitions, root='./'):
	...
	return [OutputInstruction(outputinstructiondefinition, outputdefinition, root=root) for outputinstructiondefinition in outputinstructiondefinitions]


class OutputInstruction(object):
	def __init__(self, outputinstructiondefinition, outputdefinition, root='./'):
	...
	def write(self):
		with open(os.path.join(self.root, self.filename), 'w', encoding='utf-8') as outputinstruction:
			header = 'pif %'
			...
			instructions_df.set_index('date', inplace=True)
...
class PESTControlFile(PESTControlFileType):
	...
	def __init__(self, filename, templates, parameterdefinitions, outputinstructions, modelcommand, root='./'):
	...
	def write(self):
		with open(os.path.join(self.root, self.filename), 'w', encoding='utf-8') as pestcontrolfile:
...
class SimstratPESTConfiguration(object):
	def __init__(self, simstratconfiguration, templatedefinitions, outputinstructiondefinitions, modelcommand, marker='#'):
		...
		self.root = simstratconfiguration.root

		self.templates = [generateTemplate(templatedefinition, marker, root=self.root) for templatedefinition in templatedefinitions]
		...
		self.outputinstructions = generateOutputInstructions(outputinstructiondefinitions, root=self.root)
		self.controlfile = PESTControlFile(simstratconfiguration.parfile.filename.rsplit('.', 1)[0]+'_ctr.pst', self.templates, self.parameterdefinitions, self.outputinstructions, modelcommand, root=self.root)
